...
class UsdEuroDiff:
    ...
    def get_data_from_excel(self):
        ...
        usd = {
            'date': [sheet.cell(i, 0).value for i in range(sheet.nrows)],
            'price': [sheet.cell(i, 1).value for i in range(sheet.nrows)],
            'change': [sheet.cell(i, 2).value for i in range(sheet.nrows)]
        }
        euro = {
            'date': [sheet.cell(i, 3).value for i in range(sheet.nrows)],
            'price': [sheet.cell(i, 4).value for i in range(sheet.nrows)],
            'change': [sheet.cell(i, 5).value for i in range(sheet.nrows)]
        }
    ...
    def write_data_to_excel(self, file_name):
        ...
        table_data = self.get_data_from_excel()
